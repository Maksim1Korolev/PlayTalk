...
export const connectOnline = async () => {
	io.on('connection', async socket => {
		...
		socket.on('online-ping', async username => {
			...
			socket.broadcast.emit('online-users', Array.from(onlineUserMappings.keys()))
			socket.broadcast.emit('user-connection', savedUsername, true)

			try {
				await PostUser(savedUsername, socket.id)
				console.log(`PostUser successful for ${savedUsername} with socket ID ${socket.id}.`)
			} catch (error) {
				console.error(`Error in PostUser for ${savedUsername} with socket ID ${socket.id}:`, error)
			}
		})
		...
		socket.on('on-chat-open', async receiverUsername => {
			try {
				...
				if (data && data.messageHistory) {
					...
					console.log(`Chat history sent for ${savedUsername} and ${receiverUsername}.`)
				}
			} catch (error) {
				console.error(
		...
		socket.on('send-message', async ({ senderUsername, receiverUsername, message }) => {
			...
			try {
				...
				console.error(`Error sending message from ${senderUsername} to ${receiverUsername}:`, error)
			}
		})
		...
		socket.on('disconnect', async () => {
			if (savedUsername && onlineUserMappings.has(savedUsername)) {
				...
				try {
					...
					console.log(`DeleteUser called successfully for socket ID ${socket.id}.`)
				} catch (error) {
					console.error(`Error in DeleteUser for socket ID ${socket.id}:`, error)
				}
			}
		})
	})
