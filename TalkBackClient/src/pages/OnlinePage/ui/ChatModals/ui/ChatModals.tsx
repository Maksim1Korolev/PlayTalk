...
import { ChatModalStateProps, useChatModals } from '@/pages/OnlinePage/hooks/useChatModals'
...
export const ChatModals = memo(
	({
		...
		setChatModals: React.Dispatch<React.SetStateAction<ChatModalStateProps[] | undefined>>
	}) => {
		...
		const renderChatModals = useCallback(() => {
			return chatModals?.map(({ user, position }, index) => (
				<ChatModal
		...
		}, [chatModals, className, currentUser.username, handleCloseChatModal, handleUserMessage])
